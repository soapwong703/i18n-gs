...
class i18nGS {
    ...
    async connectWithServiceAccount() {
        ...
        this.doc = new google_spreadsheet_1.GoogleSpreadsheet(this.config.spreadsheet.sheetId, auth);
    ...
    async upsertSheets(i18n) {
        ...
        async function updateExistKey(sheet, data) {
            ...
            for (const row of rows) {
                if (!(clone === null || clone === void 0 ? void 0 : clone[row.get("key")]))
                    continue;
                for (const locale in clone[row.get("key")]) {
                    ...
                    if (cell.value !== clone[row.get("key")][locale]) {
                        if (cell.value === null && !clone[row.get("key")][locale])
                            continue;
                        log_1.default.debug(`Updating ${sheet.title}/${row.get("key")}/${locale}`);
                        cell.value = (_a = clone[row.get("key")][locale]) !== null && _a !== void 0 ? _a : "";
                        updatedCount++;
                    }
                }
        ...
        try {
            for (var _c = __asyncValues(Object.entries(i18n)), _d; _d = await _c.next(), !_d.done;) {
                ...
                let sheet = (_b = this.doc.sheetsByTitle) === null || _b === void 0 ? void 0 : _b[namespace];
                if (!sheet) {
